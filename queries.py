    ...
    try:
        with connect_db() as connection:
            ...
            if avg_result and avg_result[0] is not None:
                logger.info(f"Found the avg temperature for {city_name}: {avg_result[0]}")
                return avg_result[0]
            else:
...
def highest_temperature():
    try:
        with connect_db() as connection:
            ...
            if highest_result:
                logger.info(f"Found the city with the highest temperature: {[city[0] for city in highest_result]}")
                return highest_result
            else:
                logger.warning("could not find data")
                return None
    except sqlite3.Error as e:
        logger.error(f"Failed to find the city with the highest temperature - {e}")
...
def lowest_temperature():
    try:
        with connect_db() as connection:
            ...
            if lowest_result:
                logger.info(f"Found the city with the lowest temperature: {[city[0] for city in lowest_result]}")
                return lowest_result
            else:
                logger.warning("could not find data")
                return None
    except sqlite3.Error as e:
        logger.error(f"Failed to find the city with the lowest temperature - {e}")
    ...
    try:
        with connect_db() as connection:
            ...
            if most_humid_time_result and most_humid_time_result[0] is not None:
                logger.info(f"Found the most humid time. time:{most_humid_time_result[0]} average humidity: {most_humid_time_result[1]}")
                return {
